<?php
    ...
    if (!$audit['started'][+$_GET['id']] and $audit) {
        $audit['started'][+$_GET['id']] = 1;
        mysqli_query($connection, 'UPDATE audits SET started = "'.mysqli_real_escape_string($connection, json_encode($audit['started'])).'" WHERE id = '.+$_GET['audit']);
    }
...
<div class="top">
    <div class="container">
        <div class="top__title"><span><a href="/audit?id=<?=+$_GET['audit']?>" class="back"><?=$audit['title']?></a> ⇢ (<?=+$_GET['id']?>) <?=$criteria_titles[$id]?></span></div>
    </div>
</div>
...
<style>
    * {
        --check-color: <?=$checklist_color[$id][2]?>;
        --check-color-darken: <?=$checklist_color[$id][3]?>;
        --check-color-pending: <?=$checklist_color[$id][4]?>;
        --check-color-pale: <?=$checklist_color[$id][4]?>40;
    }

    .top {
        background-color: <?=$checklist_color[$id][0]?> !important;
    }
...
<div class="content">
    <div class="container" style="max-width: 600px;">
        <h1 style="dfont-size: 1.5em;"><a href="/audit?id=<?=+$_GET['audit']?>" class="back">⇠</a> (<?=+$_GET['id']?>) <?=$criteria_titles[$id]?></h1>
        <div class="checklistform">
            ...
            <?php foreach ($checklists[$id]['categories'][0] as $i => $category): ?>
                <div class="category">
                    ...
                    <div id="category-<?=$i?>">
                        <?php foreach ($checklists[$id]['categories'][1][$i] as $criterianum): ?>
                            <form class="criteria">
                                <p class="criteria__text"><?=$criterianum+1?>. <?=$checklists[$id]['criteria'][$criterianum]?></p>
                                <div class="criteria__marks" >
                                    <div style="display: flex;">
                                        <?php if ($id !== 2 or in_array(0, $checklists[2]['custommarks'][$criterianum])): ?>
                                            <label class="criteria__mark">
                                                <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="0" hidden <?=$marks[$id][$criterianum] === 0 ? 'checked' : ''?>>
                                                <span>0,00</span>
                                            </label>
                                        <?php endif; if ($id !== 2 or in_array(0.25, $checklists[2]['custommarks'][$criterianum])): ?>
                                            <label class="criteria__mark">
                                                <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="0.25" hidden <?=$marks[$id][$criterianum] === 0.25 ? 'checked' : ''?>>
                                                <span>0,25</span>
                                            </label>
                                        <?php endif; if ($id !== 2 or in_array(0.5, $checklists[2]['custommarks'][$criterianum])): ?>
                                            <label class="criteria__mark">
                                                <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="0.50" hidden <?=$marks[$id][$criterianum] === 0.5 ? 'checked' : ''?>>
                                                <span>0,50</span>
                                            </label>
                                        <?php endif; if ($id !== 2 or in_array(0.75, $checklists[2]['custommarks'][$criterianum])): ?>
                                            <label class="criteria__mark">
                                                <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="0.75" hidden <?=$marks[$id][$criterianum] === 0.75 ? 'checked' : ''?>>
                                                <span>0,75</span>
                                            </label>
                                        <?php endif; if ($id !== 2 or in_array(1, $checklists[2]['custommarks'][$criterianum])): ?>
                                            <label class="criteria__mark">
                                                <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="1" hidden <?=$marks[$id][$criterianum] === 1 ? 'checked' : ''?>>
                                                <span>1,00</span>
                                            </label>
                                        <?php endif; ?>
                                    </div>
                                    <div style="display: flex;">
                                        <label class="criteria__mark">
                                            <input type="radio" name="mark" data-criteria="<?=$criterianum?>" value="-2" hidden <?=$marks[$id][$criterianum] === -2 ? 'checked' : ''?>>
                                        ...
                                        <?php endif; ?>
                                        <label class="criteria__icon <?=strlen($audit['comments'][$id][$criterianum]) ? 'filled' : ''?>" data-comments="<?=$criterianum?>" id="comments-activator-<?=$criterianum?>">
                                ...
                                <div class="criteria__info" id="markdesc-<?=$criterianum?>" hidden>
                                    <?php if ($id === 2): ?>
                                        <div>
                                            <?=$checklists[$id]['standard'][$criterianum]?>
                                        </div>
                                    <?php else: ?>
                                        <div>
                                ...
                                <?php if ($id !== 2): ?>
                                    <div class="criteria__info" id="standard-<?=$criterianum?>" hidden>
                                        <div>
                                            Стандарт: <?=$checklists[$id]['standard'][$criterianum]?>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                <div class="criteria__comments" id="comments-<?=$criterianum?>" hidden>
                                    <div>
                                        <textarea name="comment" placeholder="Обоснование оценки"><?=$audit['comments'][$id][$criterianum]?></textarea>
