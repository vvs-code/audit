...
Каждый аудит отображается, как отдельная строчка с градиентом, соответствующим статусу аудита.

Слева отображаются название и профиль предприятия, даты проведения аудита. 
...
В верхней части страницы находятся кнопки, которые позволяют `Создать` новый аудит или `Присоединиться` к существующему. Создавать аудит должен руководитель его группы.

<img src=".README/createaudit.png" width="48%" align="top"> <img src=".README/joinaudit.png" width="48%" align="top">

<hr>

Также в верхней части страницы отображается ссылка на личный кабинет аудитора, в котором можно изменить личные данные или сменить пароль, а также кнопка выхода из аккаунта.

![Внешний вид страницы аудита](.README/account.png)
...
<table>
    ...
    <tr>
        <td>2</td>
        <td>Производственная&nbsp;система</td>
        <td>0,1</td><td>0,1</td><td>0,1</td><td>0,1</td><td>0,1</td>
    </tr>
    <tr>
        <td>3</td>
        <td>Разработка и&nbsp;постановка на&nbsp;производство</td>
